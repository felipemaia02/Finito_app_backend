...
class UpdateUserUseCase(IUseCase[UpdateUserInput, Optional[UserResponse]]):
    ...
    async def execute(self, input_data: UpdateUserInput) -> Optional[UserResponse]:
        ...
        try:
            ...
            if input_data.user_data.email and input_data.user_data.email != existing_user.email:
                existing_email_user = await self.repository.get_by_email(input_data.user_data.email)
                if existing_email_user:
                    logger.warning(f"Email {input_data.user_data.email} already exists")
                    raise ValueError(f"Email {input_data.user_data.email} is already registered")
            ...
            updated_user = await self.repository.update(input_data.user_id, existing_user)
