...
class ExpenseCreate(BaseModel):
    ...
    amount_cents: int = Field(
        ..., gt=0, description="Amount in cents (e.g., 1000 = $10.00)"
    )
    ...
    date: Optional[datetime] = Field(
        None, description="Date of expense (defaults to now)"
    )
...
class ExpenseUpdate(BaseModel):
    ...
    spent_by: Optional[str] = Field(
        None, min_length=1, max_length=100, description="Person/entity who spent"
    )
